...
function DropdownSelect(props) {
  ...
  return (
    ...
    <div className="relative inline-flex w-full mobile-14" key={props.drpIdentity}>
      {props.disabled ?
        <button
          disabled={true}
          ref={trigger}
          className="btn btn-dropdown justify-between font-normal py-3 w-full bg-[#e9e9ea] border-[#757575] text-[#757575] px-4"
        ...
        >
          <span className="flex items-center">
            <span className='common-fonts-size font-normal mobile-14'>{options.find(option => option.id === Number(props.value)) ? options.find(option => option.id === Number(props.value)).name : 'Select'}</span>
          </span>
        ...
        <button
          ...
          ref={trigger}
          className="btn btn-dropdown justify-between font-normal py-3 w-full bg-white border-[#757575] hover:border-[#C00000] text-[#757575] px-4"
          aria-label="Select date range"
          aria-haspopup="true"
          onClick={() => setDropdownOpen(!dropdownOpen)}
          aria-expanded={dropdownOpen}
            ...
            options.map(option => {
              return (
                ...
                >
                  ...
                  <span className='font-normal mobile-14'>{option.name}</span>
                </button>
              )
            })
          }
